...
Изменения в файле **`sysctl.conf`** применяются следующей командой:
```
...
Выставление параметра **MTU**:
```
ip mtu 1400
```
> В связи с добавлением служебного заголовка появляются новые требования к допустимому значению MTU при передаче пакета

<br/>

Задание режима работы туннеля **GRE** и адресов **начала** и **конца** туннеля:
```
...
Добавление правил **`iptables`** на **ISP**:
```
...
Включение и добавление **`iptables`** в автозагрузку:
```
...
Создание **пула** для **DHCP-сервера**:
```
...
Привязываем **DHCP-сервер** к интерфейсу (который смотрит в сторону **CLI**):
```
interface int2
  dhcp-server 1
```
